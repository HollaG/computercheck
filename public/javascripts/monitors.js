...
function loadMore(all) {
    ...
    if (searchString)
        xhttp.open("GET", `/monitors/loadmore/${text}?search=${searchString}`, true);
    else
        xhttp.open("GET", `/monitors/loadmore/${text}`, true);
...
function filter(option, sliderOption, sliderOptionPrice, elem) {
    ...
    let numberFilters = Object.keys(filterObj).length
